...
public final class MessageReciever implements Runnable {
	...
	private Socket connectedSoket_;

	/**
	 * 
	 * * 外部から呼び出し不可能なコンストラクタ。 * * @param
	 * 
	 * controller * 携帯電話アプリ管理オブジェクト
	 * 
	...
	private MessageReciever(PhoneController controller, Socket socket, BufferedReader bufferedReader) {
		// フィールドの初期化
		this.controller_ = controller;
		this.connectedSoket_ = socket;
		this.buffReader_ = bufferedReader;
		this.isReading_ = true;
	}

	/**
	 * * メッセージ受信オブジェクトを生成する。 * * @param controller * 携帯電話アプリ管理オブジェクト * @param
	 * socket * メッセージを受信するソケット * @return メッセージ受信オブジェクト * @throws IOException *
	 * 入力ストリームの取得に失敗した場合にスローする
	 */

	public static MessageReciever createReciever(PhoneController controller, Socket socket) throws IOException {

		InputStream inputStream = socket.getInputStream();
		InputStreamReader inputReader = new InputStreamReader(inputStream, PhoneConstant.ENCODE_UTF_8);
		BufferedReader bufferedReader = new BufferedReader(inputReader);

		MessageReciever returnReciever = new MessageReciever(controller, socket, bufferedReader);
		return returnReciever;
	}

	/** * ソケットからメッセージを常時受信する処理。 * * @see java.lang.Runnable#run() */
	@Override
	...
	public void run() {
		...
		while (this.getReading() == true) {
			// 継続接続成立チェック
			if (connectedSoket_.getInetAddress() == null) {
				// 接続を異常終了する
			...
			if (this.controller_.isPhoneGroupMode_()) {
				ArrayList<MessageSender> msgSenderList_ = this.controller_.getMsgSenderList_();
				int skipClientNum = this.controller_.getMsgRecieverList_().indexOf(this);
