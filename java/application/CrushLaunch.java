...
 private void CrushTheBlockValidation() {  synchronized (this) {   try {    _crushPaint.get_crushController().crushTheBlock();    if (_crushPaint.get_crushController().get_crushingFlag()) {     _crushPaint._crushPaintBlock       .crushReGenarateShadowBlock(_crushPaint         .get_crushController().get_flag());     _crushPaint.get_crushController().set_crushingFlag(false);     _crushPaint.get_crushController().initFlag();    }
   } catch (Exception e) {    e.printStackTrace();   }  } }
 @Override public void run() {  while (_runGame) {   synchronized (this) {    try {     _window.validate();     _window.repaint();     CrushTheBlockValidation();     _crushPaint.set_isGG(_crushPaint.get_crushController()       .validateEndGame());     if (_crushPaint._isGG()) {      _runGame = false;     }    } catch (Exception e) {     e.printStackTrace();    }   }  } }
