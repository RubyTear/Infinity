...
 public boolean is_isGG() {  return _isGG; }
 public void set_isGG(boolean _isGG) {  this._isGG = _isGG; }
 public boolean is_isStart() {  return _isStart; }
...
 }
 public void crushReGenarateShadowBlock(byte[][] flag) {  for (int k = 0; k < flag.length; k++) {   byte[] bs = flag[k];   for (int l = 0; l < bs.length; l++) {    generateShadowBlock(l, k);   }  } }
...
 private void removeOriginalBlock() {  Component[] comps = this.getComponents();  for (int i = 0; i < comps.length; i++) {   Component component = comps[i];   if (component instanceof JLabel) {    this.remove(component);   }  } }
}
